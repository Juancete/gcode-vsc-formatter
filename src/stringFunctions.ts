...
export const intercalate: stringTransformer = (character: string) => (text: string) => {
	...
	if (position !== -1 && text.charAt(position - 1) !== ' ' && !isNaN(+text.charAt(position + 1)) && text.charAt(position - 1) !== ',') {
...
export let commandsWithTwoDigits: stringTransformer = (character: string) => (text: string) => {
	...
	const condition = (isNaN(+text.charAt(position + 2)) || text.charAt(position + 2) === ' ') && !isNaN(+text.charAt(position + 1))
